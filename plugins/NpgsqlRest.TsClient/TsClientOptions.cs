...
public class TsClientReactQueryOptions
{
    /// <summary>Enable TanStack Query hooks file generation.</summary>
    public bool Enabled { get; set; } = false;

    /// <summary>
    /// Output file path for the hooks module. In multi-file mode (BySchema
    /// or tsclient_module usage) must contain {0}, mirroring FilePath rules.
    /// Required when Enabled is true — fail fast at startup if null.
    /// </summary>
    public string? FilePath { get; set; } = null;

    /// <summary>Overwrite existing files (mirrors TsClientOptions.FileOverwrite).</summary>
    public bool FileOverwrite { get; set; } = false;

    /// <summary>Optional first segment prepended to every query key
    /// (for apps consuming multiple NpgsqlRest APIs). Null = omit.</summary>
    public string? QueryKeyPrefix { get; set; } = null;

    /// <summary>Export query key factory objects alongside hooks.</summary>
    public bool ExposeQueryKeys { get; set; } = true;

    /// <summary>
    /// Module specifier the generated hooks import TanStack Query from.
    /// Point this at an internal wrapper module when direct imports of @tanstack/react-query
    /// are not allowed; the module must re-export useQuery, useMutation, UseQueryOptions and
    /// UseMutationOptions with TanStack Query v5 semantics.
    /// </summary>
    public string ImportFrom { get; set; } = "@tanstack/react-query";

    /// <summary>Header lines for the generated hooks file
    /// (same semantics as TsClientOptions.HeaderLines).</summary>
...
public class TsClientOptions
{
    ...
    /// <summary>
    /// TanStack Query (React Query) hooks generation. When enabled, a hooks module is generated for
    /// each client module: useQuery hooks for GET endpoints and useMutation hooks for other methods,
    /// importing the client functions and @tanstack/react-query (v5 object syntax).
    /// TypeScript only — skipped with a warning when SkipTypes is true.
    /// </summary>
    public TsClientReactQueryOptions ReactQuery { get; set; } = new();
}
