...
public static partial class ConfigSchemaGenerator
{
    ...
    internal static readonly Dictionary<string, string> DescriptionFields = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        ["Auth:CookieSameSite"] = "Controls the SameSite attribute on the authentication cookie. One of: \"Strict\", \"Lax\", \"None\", \"Unspecified\". Use \"None\" for cross-origin SPA / mobile clients (browsers drop SameSite=None cookies without Secure — set CookieSecure=\"Always\" in that case). Set to null to use ASP.NET Core's default (typically \"Lax\").",
        ["Auth:CookieSecure"] = "Controls when the cookie's Secure attribute is set. One of: \"SameAsRequest\", \"Always\", \"None\". Use \"Always\" alongside CookieSameSite=\"None\" for cross-origin auth. Set to null to use ASP.NET Core's default (\"SameAsRequest\").",
        ...
        ["Auth:Schemes:CookieSameSite"] = "Per-scheme SameSite attribute. Cookies-type only. One of: \"Strict\", \"Lax\", \"None\", \"Unspecified\". Default null = inherit from the root Auth section, else ASP.NET default. Pair with CookieSecure=\"Always\" when using \"None\".",
        ["Auth:Schemes:CookieSecure"] = "Per-scheme Secure attribute policy. Cookies-type only. One of: \"SameAsRequest\", \"Always\", \"None\". Default null = inherit from the root Auth section, else ASP.NET default.",
        ...
        ["StaticFiles:ParseContentOptions:AvailableClaims"] = "List of claims types used. These will be parsed to NULL if not found in the claims collection or user is not authenticated.\nAccepts an array of claim names [\"name\",\"email\"] or an object of name->default {\"name\":\"guest\"} where the default value is substituted when the claim is absent.",
        ["StaticFiles:ParseContentOptions:AvailableEnvVars"] = "List of environment variable names whose values are templated into static content using the same {NAME} tag syntax as claims.\nResolved once at application startup (a value change requires a restart). Accepts an array of names [\"BUILD_LABEL\"] (a missing variable becomes the empty string) or an object of name->default {\"DEMO_FLAG\":\"false\"} where the default is used when the variable is absent.\nTemplates can also use the strict forms {!NAME} (same as {NAME} for listed names) and {!NAME:fallback} where the inline fallback text is used when the listed variable is unset and has no configured default. Unlisted names are always left untouched.\nValues are JSON-escaped, so templates use a bare {NAME} token with no surrounding quotes.\nSECURITY: every listed value is served to any client - never list a secret (database password, API key, signing token).",
        ...
        ["TestRunner:Filter"] = "Optional filter narrowing the discovered set — the fast path for iterating on one test: npgsqlrest ... --test --testrunner:filter=login\nMatched against each file's cwd-relative path: a value without wildcards is a substring match; with wildcards it is the same glob engine as FilePattern. Empty = run everything discovered.",
        ["TestRunner:Tag"] = "Run only test files carrying at least one of these tags (comma- or whitespace-separated; case-insensitive). A file declares tags with a `-- @tag name [name ...]` header annotation. Composes with ExcludeTag and Filter.",
        ["TestRunner:ExcludeTag"] = "Skip test files carrying any of these tags (comma- or whitespace-separated; case-insensitive; exclude wins over Tag).",
        ...
        ["TestRunner:JUnitOutput"] = "Optional path to also write a JUnit XML report (console output is always printed).",
        ["TestRunner:Keep"] = "Skip Teardown so a failed run's state can be inspected.",
        ["TestRunner:DetailedReport"] = "Detailed console REPORT: list passed assertions, print the full failing SQL statement, and show captured `raise notice` output for passing tests too.\nThis shapes the report only — for diagnostic logging of every executed query/HTTP call, raise the log channel instead (Log:MinimalLevels + LoggerName).",
        ["TestRunner:AllowEmpty"] = "Treat \"no tests discovered\" as success (exit 0) instead of exit 4.",
        ["TestRunner:Coverage"] = "Endpoint-coverage summary after the run: exercised N of M testable endpoints + the list of untested ones (endpoint kinds the runner rejects — SSE, upload, login/logout, outbound proxy — are excluded from the ratio and counted separately).\nTri-state: null (default) reports after FULL runs but stays quiet when the run is narrowed by Filter/Tag; true always reports; false never.",
        ["TestRunner:CoverageThreshold"] = "Fail an otherwise-passing run with exit 2 when endpoint coverage is below this percentage (0-100) — CI gating for \"every endpoint has a test\". Always reports coverage when set, regardless of the Coverage setting or run narrowing. Null = no gate.",
        ["Watch:Enabled"] = "Turn watch mode on (interactive/dev-only; the --watch command line flag is the shorthand for this setting).\nWith --test: re-run tests on changes (changed test file re-runs alone; changed endpoint file or database routine rebuilds endpoints in-process and re-runs everything; teardown once, on exit). Without --test: supervise the server and restart it on SQL file source, configuration, and database routine changes.",
        ["Watch:DatabasePollingInterval"] = "Watch mode (both --watch flavors): poll the database for routine changes and restart the server (server watch) or rebuild endpoints and re-run tests (test watch).\nRuns the SAME routine discovery query the endpoint source uses (same configured filters), hashed server-side — detecting exactly what changes discovered endpoints: functions/procedures, grants, COMMENT ON annotations, and the composite/table types their signatures use. Accepts \"2s\", \"500ms\", \"1m\", plain seconds, or \"hh:mm:ss\"; 0 disables.",
        ...
        ["TestRunner:Steps"] = "Named, reusable steps (name -> step; same shape as Setup/Teardown entries): \"Sql\", \"SqlFile\", \"Command\", \"WorkingDirectory\", \"ConnectionName\", \"Enabled\". Reference them by name in Setup/Teardown or from a test file's leading header comments (-- @setup Name / -- @teardown Name).\nA step with Enabled=false is simply IGNORED wherever referenced — the default configuration ships disabled example steps (create/drop test database, apply schema, run a migration tool, start/stop a Docker PostgreSQL) to flip on instead of typing.",
        ...
        ["CacheOptions:Profiles"] = "Named caching profiles. Endpoints opt into a profile via the `cache_profile <name>` comment annotation. Each profile supplies cache backend (Type), default expiration, default key parameters, and per-parameter skip-cache conditions. Backends are pooled — all profiles of the same Type share one instance. A backend is only instantiated if the type is actually used by the root or some enabled profile. Endpoints WITHOUT @cache_profile continue to use the root cache.",
        ["CacheOptions:Profiles:Enabled"] = "When false (default), this profile is ignored at startup. Set to true to register the profile. Logged at startup whether enabled or skipped.",
        ["CacheOptions:Profiles:Type"] = "Cache backend type for this profile: Memory, Redis, or Hybrid. Required when Enabled=true. Profiles sharing a Type share the same backend instance.",
        ["CacheOptions:Profiles:Expiration"] = "Default expiration for entries written under this profile (PostgreSQL interval format, e.g. '5 minutes', '30 seconds'). Used when the endpoint has no @cache_expires annotation. The annotation overrides this.",
        ["CacheOptions:Profiles:Parameters"] = "Default cache-key parameter list:\n- null/missing → use ALL routine parameters (different requests → different entries).\n- [] (empty array) → URL-only cache (one entry per endpoint, regardless of inputs).\n- [\"x\", \"y\"] → use only these named parameters as the key.\nThe endpoint's `cached p1, p2` annotation overrides this list.",
        ["CacheOptions:Profiles:When"] = "Optional list of conditional rules. Each rule has Parameter (routine param name to inspect), Value (scalar or array — JSON null matches .NET null/DBNull, others are stringify-and-equal case-insensitive), and Then (\"skip\" to bypass the cache, or a PostgreSQL interval like \"30 seconds\" to override the TTL). Rules evaluate in order; first match wins. Rules whose Parameter is not in the profile's Parameters list (or the endpoint's @cached annotation) are dropped at startup with a Warning.",
        ["CacheOptions:Profiles:When:Parameter"] = "Routine parameter name to inspect at request time (matches against ActualName or ConvertedName). Required.",
        ["CacheOptions:Profiles:When:Value"] = "Match condition. Scalar (single match) or array (OR over entries). JSON null matches .NET null/DBNull (does NOT match empty string).",
        ["CacheOptions:Profiles:When:Then"] = "Action when the rule matches: the literal string \"skip\" → bypass cache (no read, no write); or a PostgreSQL interval (e.g. \"30 seconds\", \"5 minutes\") → use this as the entry's TTL when writing.",
        ...
        ["NpgsqlRest:UseMultipleConnections"] = "Allow using multiple connections from the ConnectionStrings section. When set to true, the connection name can be set for individual Routines with the \"connection\" comment annotation.\nSome routines might use the primary database connection string, while others might want to use a read-only connection string from the replica servers.\nThis assumes the target databases have identical routine metadata (replicas, shards) - metadata is still read from a single connection. For genuinely different databases use RoutineOptions.ReadMetadataFromConnections instead, which enables multiple connections implicitly. The main connection is routable by its own name too.",
        ["NpgsqlRest:RoutineOptions:VerifyRoutedEndpoints"] = "Startup verification for routines routed by the \"connection\" annotation to a different connection than they were discovered on:\n\"None\" - no verification (default).\n\"Warn\" - one batched existence check per target connection at startup; every missing routine is logged as a warning.\n\"Fail\" - same check, but any missing routine stops the startup.\nThis checks CONTENT (does the routine exist over there?) - connectivity of every connection string is a separate check, ConnectionSettings.TestConnectionStrings (opens and closes the connection).",
        ...
        ["NpgsqlRest:WrapInTransaction"] = "When true, **every request** is wrapped in an explicit BEGIN/COMMIT, and all `set_config` calls switch to the transaction-local form (`is_local=true`).\n**Required when using a connection pooler in transaction mode** (PgBouncer transaction-pool, AWS RDS Proxy in transaction mode, Supabase Pooler) — without this, the backend can be reused across unrelated requests, allowing GUC state (and the routine call itself) to leak or split mid-request.\nDefault is false to preserve existing behavior; safe to leave off when using Npgsql's native pool only.",
        ["NpgsqlRest:JsonTimestampsAreUtc"] = "Controls how JSON datetime strings are interpreted when bound to `timestamp`, `timestamptz`, `time`, or `timetz` parameters.\nWhen true (default since 3.16.0), `Z`- and offset-bearing strings are converted to UTC and naive strings (no offset, no `Z`) are assumed UTC — making stored values identical regardless of the host process's `TZ` environment.\nWhen false, the legacy pre-3.16.0 behavior is restored: `DateTime.TryParse` interprets the string in the host's local time zone, which silently shifts stored values by the host's UTC offset on non-UTC hosts. Only set to false if you have callers that depend on the host-local interpretation of naive datetime strings and you cannot update them to send `Z`-suffixed values.",
        ["NpgsqlRest:BeforeRoutineCommands"] = "SQL commands executed after any context is set but before the main routine call. They run in the same batch as the context `set_config` calls (no extra round-trip). Each array entry is either:\n- A raw SQL string (always runs, no parameters), or\n- An object with `Enabled`, `Sql`, and optional `Parameters`. Object entries are gated by `Enabled` (default false) — set `\"Enabled\": true` to activate.\nEach parameter has a `Source` (`Claim`, `RequestHeader`, or `IpAddress`) and an optional `Name` (claim type or header name; ignored for IpAddress). Values are bound at request time via parameterized SQL.\nCommon use case: setting `search_path` from a tenant claim for multi-tenant deployments. Combine with `WrapInTransaction = true` for transaction-local scoping.",
        ["NpgsqlRest:BeforeRoutineCommands:Enabled"] = "When false (default), this command is ignored at startup. Set to true to register the command. Logged at startup whether enabled or skipped.",
        ["NpgsqlRest:BeforeRoutineCommands:Sql"] = "The SQL text to execute. May contain positional parameters ($1, $2, ...) bound from the Parameters list at request time.",
        ["NpgsqlRest:BeforeRoutineCommands:Parameters"] = "List of parameters bound to the SQL command. Each parameter has a `Source` (Claim, RequestHeader, or IpAddress) and an optional `Name` (claim type or header name; ignored for IpAddress).",
        ["NpgsqlRest:BeforeRoutineCommands:Parameters:Source"] = "Source of the parameter value. One of: `Claim` (resolves from HttpContext.User claims by Name), `RequestHeader` (resolves from request header by Name), `IpAddress` (resolves to client IP).",
        ["NpgsqlRest:BeforeRoutineCommands:Parameters:Name"] = "Source-dependent name. For `Claim`: the claim type. For `RequestHeader`: the header name. Ignored for `IpAddress`.",
        ...
        ["NpgsqlRest:AvailableEnvVars"] = "Allowlist of environment variable names available to {name} placeholder substitution in comment annotation values (response headers, custom parameters, HTTP custom type calls), alongside the routine's parameters.\nResolved once at startup (a value change requires a restart). Array form lists names (a missing variable becomes the empty string); object form maps name->default where the default is used when the variable is absent.\nAnnotation values can also use the strict forms {!NAME} (same as {NAME} for listed names) and {!NAME:fallback} where the inline fallback text is used when the listed variable is unset and has no configured default (also applies to parameters: a null parameter value yields the fallback). Unlisted names are always left untouched.\nNames are matched case-insensitively, and a routine parameter of the same name takes precedence.\nSECURITY: a value used in a RESPONSE header is sent to the client - reserve secrets (API keys, tokens) for outbound HTTP custom type calls, and use response headers only for non-secret values (e.g. server/environment name).",
        ...
        ["NpgsqlRest:WarnUnboundServerSentEventsNotices"] = "When true (default), log a one-time warning per endpoint when a RAISE at the configured SSE notice level fires inside a routine that has no @sse or @sse_publish annotation.\nNotices are logged but NOT broadcast to SSE subscribers; the warning surfaces the likely missing annotation.\nInactive when no endpoint in the build participates in SSE publishing — projects that don't use SSE pay zero overhead and see no warnings.",
        ...
        ["NpgsqlRest:RoutineOptions:ReadMetadataFromConnections"] = "Per-connection routine discovery: list of connection names from the ConnectionStrings section to read functions/procedures metadata from.\nNULL or empty (default): metadata is read from a single connection (ConnectionName or MetadataQueryConnectionName) - the current behavior.\nNon-empty: routine metadata is read from EACH listed connection, and every discovered endpoint EXECUTES on the connection it was discovered from (an explicit \"connection\" comment annotation still wins). The list replaces the default - include the main connection's name to also serve its routines. All other RoutineOptions settings and global filters are shared.\nSetting this implicitly enables multiple connections (no UseMultipleConnections flag required).\nUse this when different databases host different routines (for example OLTP + OLAP); for identical databases (replicas, shards) use UseMultipleConnections with the \"connection\" annotation instead.",
        ...
        ["NpgsqlRest:OpenApiOptions:IncludeSchemas"] = "Schema-name allow-list. When non-empty, only endpoints whose routine schema appears here are documented in the OpenAPI spec. The HTTP endpoints themselves are unaffected. Empty array (default) = document every schema.",
        ["NpgsqlRest:OpenApiOptions:ExcludeSchemas"] = "Schema-name deny-list. Endpoints whose routine schema appears here are skipped in the OpenAPI spec. Applied alongside IncludeSchemas — both must pass. Empty array (default) = no schema exclusions.",
        ["NpgsqlRest:OpenApiOptions:NameSimilarTo"] = "PostgreSQL-style SIMILAR TO pattern matched against the routine name. When set, only routines whose name matches are documented in the OpenAPI spec. `_` matches one char, `%` matches any sequence; the rest of SIMILAR TO syntax (|, *, +, ?, (...), [...]) is supported. Anchored. Default null = no name filter.",
        ["NpgsqlRest:OpenApiOptions:NameNotSimilarTo"] = "PostgreSQL-style SIMILAR TO pattern matched against the routine name for EXCLUSION in the OpenAPI spec. Same syntax as NameSimilarTo. Applied alongside it — both must pass. Default null = no name exclusion.",
        ["NpgsqlRest:OpenApiOptions:RequiresAuthorizationOnly"] = "When true, only authenticated endpoints are documented in the OpenAPI spec. Anonymous endpoints (health, login, probes) are omitted. Default false = document everything.",
        ...
        ["NpgsqlRest:McpOptions"] = "Enable or disable the MCP (Model Context Protocol) server endpoint. Disabled by default. Tools are never auto-exposed: only routines explicitly opted in with the `mcp` comment annotation become MCP tools. Implements MCP specification 2025-11-25.",
        ["NpgsqlRest:McpOptions:UrlPath"] = "URL path for the MCP endpoint (Streamable HTTP, single JSON-RPC endpoint). Default \"/mcp\".",
        ["NpgsqlRest:McpOptions:ServerName"] = "serverInfo.name reported in the MCP initialize handshake. When null, the database name from the connection string is used (falling back to \"NpgsqlRest\").",
        ...
        ["NpgsqlRest:McpOptions:Authorization:AuthorizationServers"] = "Authorization Server issuer URL(s) advertised in the Protected Resource Metadata. When empty, no PRM document is served.",
        ...
        ["NpgsqlRest:McpOptions:ToolSchemas"] = "Generation of plain function-calling schema documents (OpenAI and Anthropic tools arrays) and llms.txt from the MCP tool set. Requires at least one routine with the `mcp` annotation; produces empty documents otherwise. Independent of Enabled: the documents are generated and served even when the /mcp endpoint is disabled.",
        ["NpgsqlRest:McpOptions:ToolSchemas:Enabled"] = "Enable generation. Independent of serving the /mcp endpoint: tools are collected from `mcp` annotations either way.",
        ["NpgsqlRest:McpOptions:ToolSchemas:FileOverwrite"] = "Set to true to overwrite existing files.",
        ["NpgsqlRest:McpOptions:ToolSchemas:OpenAiFileName"] = "OpenAI Chat Completions `tools` array document file name. Null skips writing the file.",
        ["NpgsqlRest:McpOptions:ToolSchemas:OpenAiUrlPath"] = "URL path serving the OpenAI tools document. Null skips the endpoint.",
        ["NpgsqlRest:McpOptions:ToolSchemas:AnthropicFileName"] = "Anthropic Messages API `tools` array document file name. Null skips writing the file.",
        ["NpgsqlRest:McpOptions:ToolSchemas:AnthropicUrlPath"] = "URL path serving the Anthropic tools document. Null skips the endpoint.",
        ["NpgsqlRest:McpOptions:ToolSchemas:LlmsTxtFileName"] = "llms.txt markdown document file name. Null skips writing the file.",
        ["NpgsqlRest:McpOptions:ToolSchemas:LlmsTxtUrlPath"] = "URL path serving llms.txt. Null skips the endpoint.",
        ...
        ["NpgsqlRest:ClientCodeGen:OmitAutomaticParameters"] = "When true, parameters filled by the server and not settable by the client are omitted from the generated request interface, query string, and body. Covers optional automatic parameters: HTTP Custom Type fields, resolved-parameter expressions, upload metadata, and (on endpoints using user parameters) IP-address and user-claim parameters. Default is false.",
        ["NpgsqlRest:ClientCodeGen:ReactQuery"] = "TanStack Query (React Query) hooks generation. When enabled, a hooks module is generated for each client module: useQuery hooks for GET endpoints and useMutation hooks for other methods, importing the client functions and @tanstack/react-query (v5 object syntax). TypeScript only - skipped with a warning when SkipTypes is true.",
        ["NpgsqlRest:ClientCodeGen:ReactQuery:Enabled"] = "Enable TanStack Query hooks file generation.",
        ["NpgsqlRest:ClientCodeGen:ReactQuery:FilePath"] = "Output file path for the hooks module. In multi-file mode (BySchema or tsclient_module usage) must contain {0}, mirroring FilePath rules. Required when Enabled is true.",
        ["NpgsqlRest:ClientCodeGen:ReactQuery:FileOverwrite"] = "Force file overwrite.",
        ["NpgsqlRest:ClientCodeGen:ReactQuery:QueryKeyPrefix"] = "Optional first segment prepended to every query key (for apps consuming multiple NpgsqlRest APIs). Null to omit.",
        ["NpgsqlRest:ClientCodeGen:ReactQuery:ExposeQueryKeys"] = "Export query key factory objects alongside hooks.",
        ["NpgsqlRest:ClientCodeGen:ReactQuery:ImportFrom"] = "Module specifier the generated hooks import TanStack Query from. Point this at an internal wrapper module when direct imports of @tanstack/react-query are not allowed; the module must re-export useQuery, useMutation, UseQueryOptions and UseMutationOptions with TanStack Query v5 semantics.",
        ["NpgsqlRest:ClientCodeGen:ReactQuery:HeaderLines"] = "Header lines on the generated hooks file. {0} is the current timestamp.",
        ["NpgsqlRest:DartClientCodeGen"] = "Enable or disable the generation of Dart client source code files for NpgsqlRest endpoints (package:http, for Flutter projects).",
        ["NpgsqlRest:DartClientCodeGen:FilePath"] = "File path for the generated code. Set to null to skip the code generation. Use {0} to set schema (or module) name when BySchema is true",
        ["NpgsqlRest:DartClientCodeGen:FileOverwrite"] = "Force file overwrite.",
        ["NpgsqlRest:DartClientCodeGen:IncludeHost"] = "Include current host information in the baseUrl variable.",
        ["NpgsqlRest:DartClientCodeGen:CustomHost"] = "Set the custom host prefix information.",
        ["NpgsqlRest:DartClientCodeGen:CommentHeader"] = "Adds comment header above each request based on PostgreSQL routine\nSet None to skip.\nSet Simple (default) to add name, parameters and return values to comment header.\nSet Full to add the entire routine code as comment header.",
        ["NpgsqlRest:DartClientCodeGen:CommentHeaderIncludeComments"] = "When CommentHeader is set to Simple or Full, set to true to include routine comments in comment header.",
        ["NpgsqlRest:DartClientCodeGen:BySchema"] = "Create files by PostgreSQL schema. File name will use formatted FilePath where {0} is the schema name in snake case.",
        ["NpgsqlRest:DartClientCodeGen:IncludeStatusCode"] = "Set to true to wrap responses as ApiResult<T> carrying status code, response and error.",
        ["NpgsqlRest:DartClientCodeGen:SeparateModelsFile"] = "Emit request/response (and composite) model classes into a separate {name}_models.dart file. The client file imports and re-exports the models file so consumers keep a single import.",
        ["NpgsqlRest:DartClientCodeGen:ImportBaseUrlFrom"] = "Dart import URI that provides a top-level `baseUrl` String variable, instead of defining it in the module. Example: \"package:my_app/base_url.dart\".",
        ["NpgsqlRest:DartClientCodeGen:HeaderLines"] = "Header lines on each auto-generated source file. Default is [\"// autogenerated at {0}\", \"\", \"\"] where {0} is the current timestamp.",
        ["NpgsqlRest:DartClientCodeGen:SkipRoutineNames"] = "Array of routine names to skip (without schema)",
        ["NpgsqlRest:DartClientCodeGen:SkipFunctionNames"] = "Array of generated function names to skip (without schema)",
        ["NpgsqlRest:DartClientCodeGen:SkipPaths"] = "Array of url paths to skip",
        ["NpgsqlRest:DartClientCodeGen:SkipSchemas"] = "Array of schema names to skip",
        ["NpgsqlRest:DartClientCodeGen:DefaultJsonType"] = "Default Dart type for JSON values (columns and parameters of json/jsonb type)",
        ["NpgsqlRest:DartClientCodeGen:UseRoutineNameInsteadOfEndpoint"] = "Use routine name instead of endpoint name when generating function names.",
        ["NpgsqlRest:DartClientCodeGen:ExportUrls"] = "Emit top-level {name}Url functions that build the request URL.",
        ["NpgsqlRest:DartClientCodeGen:UniqueModels"] = "Keep Dart models unique, meaning models with the same fields and types will be merged into one model with the name of the first model. This significantly reduces the number of generated models.",
        ["NpgsqlRest:DartClientCodeGen:XsrfTokenHeaderName"] = "Name of the XSRF Token Header (Anti-forgery Token). This is used in multipart upload requests when Anti-forgery is enabled.",
        ["NpgsqlRest:DartClientCodeGen:ExportEventSources"] = "Emit public create{Name}EventSource factory functions for streaming (SSE) endpoints. When false, the factories are emitted with a private (underscore) name and only used internally.",
        ["NpgsqlRest:DartClientCodeGen:IncludeParseUrlParam"] = "Include optional named parameter `Uri Function(Uri uri)? parseUrl` that can rewrite the constructed URI before the request is made.",
        ["NpgsqlRest:DartClientCodeGen:IncludeParseRequestParam"] = "Include optional named parameter `http.Request Function(http.Request request)? parseRequest` (or the MultipartRequest variant for uploads) that can rewrite the constructed request before it is sent.",
        ["NpgsqlRest:DartClientCodeGen:CustomImports"] = "List of custom imports to add to the generated code. It adds a line to a file. Use full expression like `import 'package:my_app/my_type.dart';`",
        ["NpgsqlRest:DartClientCodeGen:CustomHeaders"] = "Dictionary of custom headers to add to each request in generated code. Header value is inserted verbatim (include quotes for literals).",
        ["NpgsqlRest:DartClientCodeGen:IncludeSchemaInNames"] = "When true, include PostgreSQL schema name in the generated names to avoid name collisions. Set to false to simplify names when no name collisions are expected.",
        ["NpgsqlRest:DartClientCodeGen:ErrorTypeName"] = "Name for the generated error class carrying problem-details responses. Only used when IncludeStatusCode is true.",
        ["NpgsqlRest:DartClientCodeGen:ResultTypeName"] = "Name for the generated generic result class with status, response and error fields. Only used when IncludeStatusCode is true.",
        ["NpgsqlRest:DartClientCodeGen:OmitAutomaticParameters"] = "When true, parameters filled by the server and not settable by the client are omitted from the generated request class, query string, and body. Covers optional automatic parameters: HTTP Custom Type fields, resolved-parameter expressions, upload metadata, and (on endpoints using user parameters) IP-address and user-claim parameters. Default is false.",
        ["NpgsqlRest:DartClientCodeGen:ModelPrefix"] = "Prefix for generated model class names.",
        ["NpgsqlRest:DartClientCodeGen:ModelSuffix"] = "Suffix for generated model class names.",
        ["NpgsqlRest:DartClientCodeGen:UseDateTimeType"] = "When true (default), date, timestamp and timestamptz values map to Dart DateTime (parsed with DateTime.parse, serialized with toIso8601String). When false, they map to String.",
        ...
        ["NpgsqlRest:HttpClientOptions:CacheEnabled"] = "Global kill switch for HTTP type response caching. When false, the '@cache' directive on individual types is ignored and every request fires a fresh outbound call. Caching is opt-in per type via the '@cache <interval>' type-comment directive.",
        ...
        ["NpgsqlRest:SqlFileSource:SkipPattern"] = "Glob (same semantics as FilePattern) for files to EXCLUDE from endpoint discovery.\nDefault \"*.test.sql\" so co-located SQL test files (run by the test runner) are never exposed as endpoints. Empty string disables the exclusion.",
    ...
    private static readonly Dictionary<string, string[]> EnumFields = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        ["NpgsqlRest:RoutineOptions:VerifyRoutedEndpoints"] = ["None", "Warn", "Fail"],
        ...
        ["NpgsqlRest:BeforeRoutineCommands:Parameters:Source"] = ["Claim", "RequestHeader", "IpAddress"],
        ["CacheOptions:Profiles:Type"] = ["Memory", "Redis", "Hybrid"],
        ...
        ["NpgsqlRest:DartClientCodeGen:CommentHeader"] = ["None", "Simple", "Full"],
