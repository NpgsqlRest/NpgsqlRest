...
public static partial class ConfigSchemaGenerator
{
    ...
    internal static readonly Dictionary<string, string> DescriptionFields = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        ["TestRunner:Watch"] = "Watch mode (interactive/dev-only; CLI flag: --watch): run everything once, then re-run on file changes until Ctrl+C.\nA changed test file re-runs alone; any other changed .sql under the watched tree (e.g. an included fixture) re-runs everything. Endpoint sources are not watched — restart to rebuild endpoints. Teardown runs once, on exit; a graceful stop exits 0 (not for CI gating).",
