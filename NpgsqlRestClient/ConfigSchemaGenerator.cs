...
public static partial class ConfigSchemaGenerator
{
    ...
    internal static readonly Dictionary<string, string> DescriptionFields = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        ["TestRunner:Verbose"] = "Show captured `raise notice` output for all tests, not just failed/errored ones.",
        ...
        ["TestRunner:Setup"] = "Run-once setup, BEFORE endpoint discovery. Command steps run first (provision infra), then SqlFile/Sql steps (migrations/seed) — declared order preserved within each group.\nEach entry: { \"Command\": \"...\", \"WorkingDirectory\": \"...\" } | { \"SqlFile\": \"...\" } | { \"Sql\": \"...\" }.",
        ["TestRunner:Teardown"] = "Run-once teardown, ALWAYS (best-effort), reverse order: SqlFile/Sql first, then Command (e.g. docker down). The `Keep` option skips it.",
