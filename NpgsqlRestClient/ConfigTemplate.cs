...
{
    /// <summary>
    /// Full JSONC template for --config output.
    /// This is the exact content of appsettings.json with all comments preserved.
    /// </summary>
    internal const string DefaultConfigJsonc = """
    {
      //
      ...
      "TestRunner": {
        ...
        "AllowEmpty": false,
        //
        // Watch mode (interactive/dev-only; CLI flag: --watch): run everything once, then re-run on file
        // changes until Ctrl+C. A changed test file re-runs alone; a changed ENDPOINT file (SqlFileSource
        // FilePattern) rebuilds the endpoints in-process (delta reported; ErrorMode forced Exit->Skip so a
        // broken file cannot kill the session) and re-runs everything; any other changed .sql under the test
        // tree (e.g. an included fixture) re-runs everything. Teardown runs once, on exit; a graceful stop
        // exits 0 (not for CI gating).
        //
        "Watch": false,
        //
        ...
        // from the ratio and counted separately). CoverageThreshold (0-100, implies Coverage) fails an
        // otherwise-passing run with exit 2 when coverage is below it — CI gating for "every endpoint has a test".
        //
        "Coverage": false,
