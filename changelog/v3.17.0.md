...
**Configuration** — `NpgsqlRest:McpOptions`, disabled by default, surfaced in `--config`, `--config-schema`, and the JSON schema: `Enabled`, `UrlPath` (`/mcp`), `ServerName` (null → database name → `"NpgsqlRest"`), `ServerVersion` (`"1.0.0"`), `Instructions`, `ToolDescriptionSuffix`, `AllowedOrigins`, and the `Authorization` object (`RequireAuthorization`, `AuthorizationServers`, `ScopesSupported`, `Audience`, `ProtectedResourceMetadataPath`).
...
- `tools/list` lists every opted-in tool and does not yet filter to the caller's permitted subset (a SHOULD in the spec); authorization is still enforced on `tools/call`.
