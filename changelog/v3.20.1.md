...
- **Variables already present in the process environment always win** — the file only fills in missing ones (the standard dotenv convention). ⚠️ This is a behavior change for existing `EnvFile` users: before 3.20.1 the file overwrote the environment. Within the file itself, a repeated key keeps its last value.
...
- Downloads the three skill files (`SKILL.md`, `annotations-reference.md`, `configuration-reference.jsonc`) from the **release branch matching the running version** (e.g. `3.20.1`), so the skill references always match the installed binary. When that branch is not available, it falls back to `master` with a printed note.
