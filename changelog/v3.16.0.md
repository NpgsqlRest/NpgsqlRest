...
`DateOnly.TryParse` rejects `Z`- and offset-bearing strings outright (verified across `UTC`, `America/Los_Angeles`, `Europe/Zagreb`, `Pacific/Auckland`) — it does not silently shift, so the `date` parser is not affected.
