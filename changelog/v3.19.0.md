...
The `--watch` flag runs in one of **two modes**, depending on whether `--test` is present:

| Command | Mode | Watches | On change |
|---|---|---|---|
| `npgsqlrest ... --test --watch` | **Test watch** | test files, included fixtures/profiles, and — when the SQL file source is enabled — the endpoint SQL files | changed test re-runs alone; endpoint change rebuilds endpoints in-process and re-runs everything |
| `npgsqlrest ... --watch` | **Server watch** | the SQL file source tree and the configuration files | the server restarts (~1s) |

Test watch works with or without a SQL file source (database-routine endpoints are testable too — they are just not watchable) and is described in the test runner section above. Server watch requires an **enabled SQL file source** — `--watch` without it (and without `--test`) has nothing to watch and exits with an error.
