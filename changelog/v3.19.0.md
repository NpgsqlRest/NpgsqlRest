...
/* GET /api/get-users */
select (select jsonb_array_length(body::jsonb) from _response) = 5, 'seed + fixture users listed';
...
"TestRunner": {
  ...
  "Watch": false,                       // watch mode (CLI: --watch): re-run on file changes until Ctrl+C; teardown on exit
  "Coverage": false,                    // endpoint-coverage summary: exercised N/M testable + the untested list
  "CoverageThreshold": null,            // 0-100 (implies Coverage): fail an otherwise-passing run (exit 2) below it
