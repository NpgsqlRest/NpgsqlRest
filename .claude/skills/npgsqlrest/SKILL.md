...
- Placeholders `{name}` in URL/headers/body substitute from params, resolved-param expressions, and allowlisted env vars. Env vars are opt-in via `NpgsqlRest:AvailableEnvVars` (array of names, or object of `name → default`; resolved once at startup, matched case-insensitively; a same-named routine parameter wins) — ideal for API keys: `Authorization: Bearer {WEATHER_API_KEY}`. Strict forms (3.21.0+): `{!name}` resolves like `{name}` for known names; `{!name:fallback}` substitutes the inline fallback when the listed var is unset with no configured default, or when a parameter value is null (`X-Plan: {!_plan:free}`); unlisted names stay literal in every form.
...
- `{ENV_VAR}` placeholders are substituted from environment variables at startup (left untouched when unset). `{!ENV_VAR}` (3.20.0+) marks the variable **required** — startup fails naming the missing variable. `{!ENV_VAR:fallback}` (3.21.0+) uses the literal fallback text when unset — never fails (fallback runs from the first `:` to the closing `}`, so it may contain `:`; only the `{!` form takes a fallback, a plain `{NAME:...}` stays literal). The default connection string ships with fallbacks matching PostgreSQL defaults (`{!PGHOST:localhost}`, `{!PGPORT:5432}`, `{!PGUSER:postgres}`, `{!PGPASSWORD:postgres}`) and only `{!PGDATABASE}` required — setting just `PGDATABASE` connects to a stock local PostgreSQL.
- `Config:EnvFile` defaults to `"./.env"` (3.21.0+): a `.env` file next to the config (working directory) is loaded at startup — so the minimal setup is the default config plus a one-line `.env` (`PGDATABASE=mydb`). Real environment variables always win over the file; the missing default file logs Information (a missing *custom* path logs a Warning); set to `null` to disable.
- **Multiple connections** (named `ConnectionStrings` entries): two mechanisms. For **identical** databases (replicas/shards): `UseMultipleConnections: true` + per-endpoint `@connection Name` annotation — metadata still read from ONE connection, the target is assumed to have the same routine (opt-in startup check: `RoutineOptions:VerifyRoutedEndpoints: Warn|Fail`, 3.21.0+). For databases hosting **different** routines (OLTP + OLAP): `RoutineOptions:ReadMetadataFromConnections: ["Default", "olap"]` (3.21.0+) — routine metadata is read from EACH listed connection and endpoints execute where discovered (`@connection` wins; list replaces the default, so include the main name; implicitly enables multiple connections; same-path collisions warn, later connection wins). The main connection is routable by its own name.
