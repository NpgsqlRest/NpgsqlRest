...
- `Config:EnvFile` defaults to `"./.env"` (3.20.1+): a `.env` file next to the config (working directory) is loaded at startup — so the minimal setup is the default config plus a one-line `.env` (`PGDATABASE=mydb`). Real environment variables always win over the file; the missing default file logs Information (a missing *custom* path logs a Warning); set to `null` to disable.
